...
MyAudio::~MyAudio()
{
	...
	if ((paErr = Pa_Terminate()) != paNoError)
		std::cerr << "PortAudio : terminate failed" << Pa_GetErrorText(paErr) << std::endl;
}
...
bool MyAudio::close()
{
	...
	if ((paErr = Pa_CloseStream(_stream)) != paNoError)
		std::cerr << "PortAudio : Close stream failed " << Pa_GetErrorText(paErr) << std::endl;
...
bool MyAudio::start()
{
	...
	if ((paErr = Pa_StartStream(_stream) != paNoError))
		std::cerr << "PortAudio : Can't start stream " << Pa_GetErrorText(paErr) << std::endl;
	return paErr == paNoError;
}
...
bool MyAudio::stop()
{
	...
	if ((paErr = Pa_StopStream(_stream) != paNoError))
		std::cerr << "PortAudio : Can't stop stream " << Pa_GetErrorText(paErr) << std::endl;
	return paErr == paNoError;
}

int MyAudio::playCallback(float *ptr, unsigned long frameCount)
{
	if (_buffer.size() >= frameCount * CHANNEL * 100) {
...
int MyAudio::recordCallback(float *ptr, unsigned long frameCount)
{
	...
	// if (_buffer.size() >= frameCount * CHANNEL * 300)
	// 	return paComplete;
	return paContinue;
}